...
function sourceRegistry(): Record<string, SourceRegistryRecord> {
  ...
  return {
    "source-document": {
      ...
      kind: "psd-document",
      displayName: "nexus.psd",
      version: 1,
      refresh: normal,
      locator: {
        locatorId: "linked:source-document",
        kind: "linked-file",
        suggestedFileName: "nexus.psd",
        relativePathHint: null,
      },
      contentFingerprint: null,
      data: {
        importSettings: {
          compositionName: "Nexus",
    ...
    "source-node": {
      ...
      kind: "psd-node",
      displayName: "Nexus Pixel",
      version: 1,
      refresh: normal,
      data: {
        documentSourceId: "source-document",
        sourceKey: "layer:nexus-pixel",
        sourcePath: "nexus.psd/Pixel",
...
): Extract<LayerDocumentNexusTransitionResult, { ok: true }> {
  const result = reduceLayerDocumentNexus(state, action);
  assert.equal(result.ok, true);
  if (!result.ok) throw new Error(result.error.message);
  return result;
}
...
const editorNexusStateRef = {
  current: initialize(),
};
const editorNexus = createEditorNexusPort(
  editorNexusStateRef.current,
  reduceLayerDocumentNexus,
  () => {
    editorNexusStateRef.current =
      editorNexus.state;
...
const editorNexusSelectionChanged =
  editorNexus.selectSource({
      kind: "library-source",
      sourceId: "source-document",
  });
assert.equal(editorNexusSelectionChanged.ok, true);
if (!editorNexusSelectionChanged.ok) {
  throw new Error(
    editorNexusSelectionChanged.error.message
  );
}
assert.strictEqual(
  editorNexus.state,
...
assert.deepEqual(
  editorNexus.state.session.sourceSelection,
  {
    kind: "library-source",
    sourceId: "source-document",
  }
...
assert.equal(
  editorNexusSelectionChanged.effect.recomputeRender,
  false
);
...
const invalidScope = reduceLayerDocumentNexus(
...
const livePort = createEditorNexusPort(
  initialize(),
  reduceLayerDocumentNexus
);
assert.equal(
...
const liveFirstResult = livePort.commitLayerTransaction(
  liveFirstTransaction
);
...
const liveSecondResult = livePort.commitLayerTransaction(
  liveSecondTransaction
);
  ...
  buildUpdateLayerDocumentDomainTransaction(
    domainInitial.currentProject,
    {
      ...
      update: {
        ...
        data: {
          documentVersion: 2,
          elements: [{ kind: "stroke", tool: "brush", color: "#222222", size: 4,
...
if (refreshSource.kind !== "psd-node") {
  throw new Error("Invalid refresh nexus fixture");
}
...
const staleRejected = reduceLayerDocumentNexus(
...
const invalidAfter = reduceLayerDocumentNexus(
  invalidInitial,
  {
    kind: "commit-layer-transaction",
    transaction: {
...
const nonPlainRejected = reduceLayerDocumentNexus(
  invalidInitial,
  {
    kind: "commit-layer-transaction",
    transaction: {
...
const noChangeRejected = reduceLayerDocumentNexus(
  invalidInitial,
  {
    kind: "commit-layer-transaction",
    transaction: {
...
const mismatchedHistoryRejected = reduceLayerDocumentNexus(
  invalidInitial,
  {
    kind: "commit-layer-transaction",
    transaction: {
...
const taskFiles = [
  ...
  "src/engines/project/helpers/layerDocumentNexusHelpers.ts",
  "src/engines/project/actions/layerDocumentNexusTransitionHelpers.ts",
  "src/engines/project/actions/layerDocumentNexusLayerCommitReducer.ts",
  "src/engines/project/actions/layerDocumentNexusSourceCommitReducer.ts",
  "src/engines/project/actions/layerDocumentNexusHistoryReducer.ts",
  "src/engines/project/actions/layerDocumentNexusRuntimeSessionReducer.ts",
...
const editorNexusFiles = [
  "src/editor/nexus/models/editorNexusModel.ts",
  "src/editor/nexus/helpers/editorNexusPortHelpers.ts",
  "src/editor/nexus/useEditorNexus.ts",
  "src/editor/nexus/index.ts",
];
editorNexusFiles.forEach((path) => {
  const source = readFileSync(path, "utf8");
  assert.doesNotMatch(
    source,
...
const editorNexusHookSource = readFileSync(
  "src/editor/nexus/useEditorNexus.ts",
  "utf8"
);
assert.match(
  editorNexusHookSource,
...
assert.doesNotMatch(
  ...
  /useLayerDocumentNexus|reduceLayerDocumentNexus/
);

console.log("Layer Document Project nexus verification passed");

...
- 모든 Markdown 문서는 `docs/` 아래에 둔다.
- `docs/00_rule.md`는 변하지 않는 철학과 운영 원칙만 담는다.
...
- 변경 성격에 맞는 lint, test, build와 `git diff --check`를 수행한다.
- 정적 검증은 실제 Browser QA 통과를 의미하지 않는다.
- QA는 사용자가 명시적으로 요청했을 때만 수행한다.
- 요청된 Browser QA는 headless를 우선한다.
- 실제 Chrome QA는 사용자가 명시적으로 요청한 경우에만 기존 Chrome
  환경의 새 창에서 수행한다.
- QA를 위해 별도 브라우저 앱이나 사용자 프로필을 만들지 않는다.
- 500줄 이상인 TypeScript 파일은 이름과 줄 수만 리팩토링 후보로 보고하며
  승인 없이 분해하지 않는다.

## 12. 서브에이전트 운영

- 감독관은 루트 에이전트이며 서브에이전트는 작업자다.
- 작업자는 맡은 Task만 수행하고 Sprint, QA와 다음 Task를 결정하지 않는다.
- 작업자는 다른 작업자의 상태를 관리하지 않는다.
- 문제가 생기면 임의로 범위를 확장하지 않고 감독관에게 보고한다.
- 기본 작업자는 한 명이며 하위 서브에이전트를 만들지 않는다.
- 응답이 늦다는 이유만으로 재촉, 중단 또는 교체하지 않는다.
- 작업자는 조사·구현·검증 완료나 진행 불가처럼 안전한 단계가 바뀔 때
  보고한다.
- 작업자는 `docs/99_recent_task.md`를 수정하지 않는다.
- 감독관은 작업을 계속하는 동안 매 Task마다 99를 작성하지 않는다.
- 감독관은 작업을 멈출 때 가장 최근 Task 한 건으로 99 전체를 교체한다.

...
Library의 Source 작업 Runtime owner는 다음과 같다.

| 흐름 | owner | Project 교체·unmount cleanup |
|---|---|---|
...
소유하며 실제 Project 변경은 계속 Project Owner command/transaction을 통한다.
...
Source와 Audio Layer를 Owner transaction 한 번으로 생성한다. 같은 PCM fingerprint의
...
수 있다. Owner commit 뒤 Runtime registration 재시도가 남은 상태에서 Project가

...
import type { ProjectStorageTarget } from "@/gateway/contracts/projectStorageGateway";
...
export function createProjectLifecycleUiCommandPort(
  ...
  return {
    ...
    newProject: async ({
      ...
      let imported = true;
      if (psdSources.length > 0 && options.importPsdSources) {
        imported =
          await options.importPsdSources(psdSources);
      }
      ...
      setNotice({
        ...
        message: !imported
          ...
          : psdSources.length > 0
            ? `${projectName}.ziq 프로젝트를 만들고 PSD ${psdSources.length}개를 불러왔습니다.`

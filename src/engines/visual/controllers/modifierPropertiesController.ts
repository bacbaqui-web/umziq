...
} from "@/engines/visual/helpers/propertiesModifierHelpers";
import { parsePropertiesNumericDraft } from "@/engines/visual/helpers/propertiesNumericHelpers";
import type {
...
export function createModifierPropertiesController(
  ...
  const mouthRepetitionsInputId =
    "modifier.mouth-basic.repetitionsPerSecond" as const;
  ...
  const normalizeMouthRepetitions = (
    value: number
  ) => Math.min(
    12,
    Math.max(0.5, Math.round(value * 2) / 2)
  );
  const setMouthBasicRepetitions = (
    ...
    const repetitionsPerSecond =
      normalizeMouthRepetitions(requestedValue);
    if (
  ...
  const commitMouthBasicRepetitions = () => {
    syncDraftScope();
    const draftState = port.draft?.read();
    const rawValue = draftState?.inputDrafts[
      mouthRepetitionsInputId
    ];
    if (
    ...
    const result = setMouthBasicRepetitions(
      parsed.value
    );
    if (result?.ok) {
      port.draft?.finish(
  ...
  return {
    ...
    setMouthBasicRepetitionsPerSecond:
      setMouthBasicRepetitions,
    focusMouthBasicRepetitions: () => {
      const scopeIdentity = syncDraftScope();
      const modifier = readEditableDescriptor()
        ?.modifiers.find(
          (candidate) =>
            candidate.type === "mouth-basic"
        );
      if (!modifier || modifier.type !== "mouth-basic" || !port.draft) return false;
      port.draft.begin(
        mouthRepetitionsInputId,
        String(modifier.repetitionsPerSecond ?? 4),
        scopeIdentity
      );
      return true;
    },
    changeMouthBasicRepetitions: (
      ...
      if (
        !port.draft ||
        port.draft.read().focusedInputId !==
          mouthRepetitionsInputId ||
        parsePropertiesNumericDraft(value)
          .kind === "invalid"
      ) return false;
      return port.draft.change(
        mouthRepetitionsInputId,
        value
      );
    },
    blurMouthBasicRepetitions:
      commitMouthBasicRepetitions,

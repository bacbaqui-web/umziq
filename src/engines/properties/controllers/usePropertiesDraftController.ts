...
export function usePropertiesDraftController({ scope, state }: Options): PropertiesDraftControllerPort {
  ...
  const focusedInputId = isCurrentScope
    ? state.focusedNumericInputId as PropertiesNumericInputId | null
    : null;

  const getNumericDraft = useCallback((inputId: PropertiesNumericInputId) => {
    if (!isCurrentScope) return undefined;
    return state.numericDrafts[inputId];
  }, [isCurrentScope, state.numericDrafts]);

  const hasNumericDraft = useCallback((inputId: PropertiesNumericInputId) => (
    isCurrentScope && Object.prototype.hasOwnProperty.call(state.numericDrafts, inputId)
  ), [isCurrentScope, state.numericDrafts]);

  const focusNumericDraft = useCallback((inputId: PropertiesNumericInputId) => {
  ...
  const setNumericDraft = useCallback((inputId: PropertiesNumericInputId, value: string) => {
    state.setNumericDrafts((current) => ({ ...current, [inputId]: value }));
  }, [state]);

  const clearNumericDraft = useCallback((inputId: PropertiesNumericInputId) => {

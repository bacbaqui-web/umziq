...
import { useAudioEngine } from "@/engines/audio";
import {
...
} from "@/editor/useLayerDocumentEditorNexus";
import {
...
import { useDrawingEngine } from "@/engines/drawing";
import {
  createWebMenuPlatformPorts,
  createWebMicrophoneCaptureGateway,
  createWebExportDestinationGateway,
} from "@/gateway";

export function useEditorRoot():
EditorShellLayoutProps {
  const shell = useEditorShellLayoutState();
  const nexus = useLayerDocumentEditorNexus();
  const canvasState = useEditorCanvasRuntimeState(
    PREVIEW_MIN_WORKSPACE_WIDTH,
    PREVIEW_MIN_WORKSPACE_HEIGHT,
    nexus.state.currentProject.payload.canvasSettings,
    (settings) => nexus.commitCanvasSettings(
      settings,
      "촬영 범위 설정 변경"
    )
  );
  const runtime =
    useLayerDocumentEditorRuntime(nexus);
  const menuPlatform = useMemo(
    () => createWebMenuPlatformPorts(),
    []
  );
  const sourceAccess = runtime.sourceAccess;
  ...
  const timelineTimingDraftRuntime = useMemo(
    () =>
      createLayerDocumentTimelineTimingDraftRuntime(),
    []
  );
  const panelPorts =
    useLayerDocumentPanelEnginePorts({
      nexus: runtime.nexus,
      nexusCommands: runtime.nexusCommands,
  ...
  const timeline =
    useLayerDocumentTimelineEngine({
      nexus: panelPorts.timelineNexus,
      ...
      resetRevision:
        runtime.nexusEffect.localUiRevision,
  ...
  const visual =
    useLayerDocumentVisualEngine({
      ...
      resetRevision:
        runtime.nexusEffect.localUiRevision,
      mouthBasic: {
        readProject: () => runtime.nexus.state.currentProject,
  ...
  const audioInspector = useAudioEngine({
    port: panelPorts.audioEffects,
    basicPort: panelPorts.audioBasic,
    resetRevision: runtime.nexusEffect.localUiRevision,
  });
  const libraryPreview = useMemo(() =>
    createLayerDocumentLibraryPreviewReader({
      ...
      resources: runtime.resources,
      readAudioWaveform: runtime.audio.readWaveform,
    }), [runtime.audio, runtime.nexus, runtime.resources]);
  ...
  const library =
    useLayerDocumentLibraryEngine({
      ...
      resetRevision: runtime.nexusEffect.localUiRevision,
      layerCommands: {
        createDrawing: panelPorts.libraryLayerCommands.createDrawing,
        duplicate: panelPorts.libraryLayerCommands.duplicate,
        convertToDrawing: panelPorts.libraryLayerCommands.convertToDrawing,
      },
    });
  ...
  const canvas =
    useLayerDocumentCanvasEngine({
      ...
      shortformFrameHeight:
        SHORTFORM_FRAME_HEIGHT,
      projectId:
        runtime.nexus.state.currentProject.metadata.projectId,
      ...
      resetRevision:
        runtime.nexusEffect.revision,
    });
  const drawing = useDrawingEngine({
    selected: panelPorts.drawing.readSelected(),
    parentSize: { width: scope.model.activeGroup.data.width, height: scope.model.activeGroup.data.height },
    resetRevision: runtime.nexusEffect.localUiRevision,
    replaceElements: panelPorts.drawing.replaceElements,
  });
  ...
  const fullResolutionProjectRenderer = useMemo(
    () =>
      createFullResolutionProjectRenderer({
        readProject: () =>
          runtime.nexus.state.currentProject,
        resources: runtime.resources,
  ...
  return {
    ...
    libraryProps: {
      ...library.viewProps,
      registerSourceFiles: sourceAccess.registerFiles,
    },
    previewPaneProps: { ...canvas.viewProps, drawing },
    visualPanelProps:
      visual.viewProps,
    audioPanelProps: {
      effects: audioInspector.viewProps,
      basic: audioInspector.basic,
    },
    timelinePanelProps:
      timeline.viewProps,
    menuProps: {
      ...
      exportOptions: {
        projectName:
          runtime.nexus.state.currentProject.metadata.name,
        prepare: () =>
          panelPorts.libraryController.openProject(),
        destinationPort: exportDestination,
        durationFrames:
        ...
        runtime: {
          ...
          run: (format, destination, onProgress, signal) =>
            exportProject({
              ...
              playback: runtime.playback,
              project: runtime.nexus.state.currentProject,

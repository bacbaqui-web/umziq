...
### `98_sprint_plan.md` 작성 규칙

- `98`은 구현에 필요한 설계와 제약을 충분히 제공하되 반복 설명을 최소화한다.
- 모든 Task에 적용되는 철학과 제약은 문서 앞의 공통 규칙에 한 번만 작성한다.
- `00_rule.md`와 영구 Architecture 문서에 이미 정의된 내용은 다시 풀어 쓰지 않고 문서명만 참조한다.
- 각 Task에는 목적, 작업 내용, 정적 검증, 완료 조건과 해당 Task만의 추가 규칙만 작성한다.
- Task에서 공통 규칙을 반복하지 않으며 새로운 내용이 있을 때만 설명을 추가한다.
- Gate에는 `PASS` 또는 `FAIL`, 발견한 문제, 수정 사항, 다음 Task 진행 여부만 기록한다.
- 구현 품질과 판단 근거는 유지하되 같은 철학, 제약, 완료 조건을 여러 위치에 중복 기록하지 않는다.
...
- 요청된 QA는 헤드리스 브라우저를 기본으로 수행한다.
- 실제 Chrome QA는 사용자가 명시적으로 요청한 경우에만 기존 Chrome 프로필의 새 창에서 수행한다.
- 실제 Chrome이 필요한 결함을 발견해도 임의로 실행하지 않고 필요성을 먼저 보고한다.
- QA를 위해 별도 브라우저 앱 인스턴스나 별도 사용자 프로필을 만들지 않는다.
- Chrome이 꺼져 있으면 필요할 때 실행하고, QA 종료 후에는 QA용으로 연 창만 닫으며 기존 창과 탭은 건드리지 않는다.
...
- QA는 사용자가 명시적으로 요청했을 때만 실행하며 위 브라우저 QA 규칙을 따른다.
